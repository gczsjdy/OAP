...
import org.apache.spark.shuffle.sort.remote.{HadoopFileSegmentManagedBuffer, MessageForHadoopManagedBuffers, RemoteHadoopShuffleDataIO, RemoteShuffleBlockResolver}
      ...
      case openBlocks: OpenBlocks =>
        ...
        val isShuffleRequest = (blocksNum > 0) &&
          ...
          (SparkEnv.get.conf.get(
            "spark.shuffle.sort.io.plugin.class", classOf[RemoteHadoopShuffleDataIO].getName)
            == classOf[RemoteHadoopShuffleDataIO].getName)
        if (isShuffleRequest) {
          val blockIdAndManagedBufferPair =
            openBlocks.blockIds.map(block => (block, RemoteShuffleBlockResolver.active.getBlockData(
