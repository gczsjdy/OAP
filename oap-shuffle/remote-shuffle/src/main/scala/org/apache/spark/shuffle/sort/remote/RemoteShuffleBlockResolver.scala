...
import org.apache.hadoop.fs.{FileSystem, FSDataInputStream, Path, PathFilter}
 ...
 *
 */
class RemoteShuffleBlockResolver(val conf: SparkConf) extends ShuffleBlockResolver with Logging {

  RemoteShuffleBlockResolver.setActive(this)
  ...
  lazy val fs = new Path(dirPrefix).getFileSystem(getHadoopConf)
  ...
  private[remote] lazy val remoteShuffleTransferService: BlockTransferService = {
    ...
    new RemoteShuffleTransferService(
      ...
      env.blockManager.blockManagerId.host,
      env.conf.get(config.BLOCK_MANAGER_PORT),
  ...
  private[spark] val getHadoopConf = {
    val storageMasterUri = conf.get("spark.shuffle.remote.storageMasterUri")
    ...
    if (storageMasterUri.startsWith("hdfs")) {
      ...
      val port = conf.get(RemoteShuffleConf.STORAGE_HDFS_MASTER_UI_PORT)
    ...
    (new SparkHadoopUtil).appendS3AndSparkHadoopHiveConfigurations(conf, hadoopConf)
    appendRemoteStorageHadoopConfigurations(conf, hadoopConf)
    hadoopConf
  }
  ...
  def writeIndexFileAndCommit(
    ...
    val indexTmp = tempPathWith(indexFile)
    try {
  ...
  override def getBlockData(blockId: BlockId, dirs: Option[Array[String]]): ManagedBuffer = {
    ...
    val indexFile = getIndexFile(shuffleBlockId.shuffleId, shuffleBlockId.mapId)

    val (offset, length) =
      if (indexCacheEnabled) {
        val shuffleIndexInfo = shuffleIndexCache.get(indexFile)
        val range = shuffleIndexInfo.getIndex(shuffleBlockId.reduceId)
        ...
        in.seek(shuffleBlockId.reduceId * 8L)
        try {
          val offset = in.readLong()
          val nextOffset = in.readLong()
          val actualPosition = in.getPos()
          val expectedPosition = shuffleBlockId.reduceId * 8L + 16
  ...
  def removeDataByShuffleId(shuffleId: Int): Unit = {
    val file = getDataFile(shuffleId, 0)
    fs.listStatus(new Path(dirPrefix), new PathFilter {
      override def accept(path: Path): Boolean =
        path.toString.startsWith(file.toString.substring(0, file.toString.indexOf('_') + 1))
  ...
  private def getPath(blockId: BlockId): Path = {
    new Path(s"${dirPrefix}/${blockId.name}")
  }
  ...
  /**
    ...
    * returning Path
    */
  private[remote] def createTempShuffleBlock(): (TempShuffleBlockId, Path) = {
    var blockId = TempShuffleBlockId(UUID.randomUUID())
    val tmpPath = getPath(blockId)
...
private[remote] class RemoteShuffleIndexInfo extends Logging {
  ...
  def this(indexFile: Path) {
    this()
    val fs = RemoteShuffleBlockResolver.getFileSystem
...
object RemoteShuffleBlockResolver {
  var active: RemoteShuffleBlockResolver = _
  private[remote] def setActive(update: RemoteShuffleBlockResolver): Unit = active = update
