...
object RemoteShuffleBlockIterator {
  ...
  private val maxConcurrentFetches =
    RemoteShuffleBlockResolver.active.conf.get(RemoteShuffleConf.NUM_CONCURRENT_FETCH)
  private val eagerRequirement =
    RemoteShuffleBlockResolver.active.conf.get(RemoteShuffleConf.DATA_FETCH_EAGER_REQUIREMENT)
